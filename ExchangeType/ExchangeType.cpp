...
Exchange ExchangeType::toExchange(const std::string& str) {
    ...
    if (!exchange_1) RUNTIME_ERROR("Environment variable EXCHANGE_1 is not set");
    if (!exchange_2) RUNTIME_ERROR("Environment variable EXCHANGE_2 is not set");

    if (exchange_1 && str == exchange_1) return Exchange::EXCHANGE_1;
    if (exchange_2 && str == exchange_2) return Exchange::EXCHANGE_2;
